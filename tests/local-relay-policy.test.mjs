...
test("the default policy is wildcard and accepts a public Minecraft target", () => {
...
test("the default wildcard accepts multiple public Minecraft host and IP shapes", () => {
  const policy = minecraftTargetPolicy();
  for (const [host, port] of [
    ["mc.belenko.dev", 25565],
    ["play.example.net", 25566],
    ["1.1.1.1", 25565],
    ["2001:4860:4860::8888", 25565],
  ]) {
    assert.equal(policy.allows(host, port), true, `${host}:${port}`);
    assert.equal(resolveMinecraftTarget(host, port, policy).port, port);
  }
});
