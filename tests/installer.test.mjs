...
const BINARYEN_PLATFORM_NAMES = [
  "arm64-macos", "x86_64-macos", "aarch64-linux", "x86_64-linux", "arm64-windows", "x86_64-windows",
];
...
test("Windows builds recover the verified llvm-mingw adapter environment", async (t) => {
  ...
  assert.match(sea, /creating.*as a local compatibility adapter/);
  assert.match(sea, /verified Node runtime copy/);
  assert.match(sea, /embedded the local adapter payload in the Node copy/);
  assert.match(sea, /quiet: true/);
  assert.match(clShim, /MSVC-compatible shim Version 19\.44\.35228 for x64/);
  assert.match(clShim, /could not start \$\{cc\}/);
  assert.match(clShim, /encoding: "utf8", windowsHide: true/);
  assert.doesNotMatch(clShim, /stdio: "inherit"/);
...
test("developer-tool URLs are exact HTTPS vendor paths and reject unsafe variants", () => {
  ...
  for (const platform of BINARYEN_PLATFORM_NAMES) {
    const archive = `https://github.com/WebAssembly/binaryen/releases/download/version_131/binaryen-version_131-${platform}.tar.gz`;
    assert.equal(toolUrl(archive, "binaryen"), archive);
    assert.equal(toolUrl(`${archive}.sha256`, "binaryen"), `${archive}.sha256`);
  }
  ...
  assert.throws(() => toolUrl(BINARYEN_ARCHIVE_URL.replace("arm64-macos", "unknown-linux"), "binaryen"), /pinned vendor path/);
