...
import { completeMicrosoftBrowserAuth } from "./microsoft-browser-auth.mjs";
import {
  WORKER_RELAY_MIN_SECRET_LENGTH,
  WORKER_RELAY_URL,
  WorkerTcpUpstream,
} from "./worker-tcp-upstream.mjs";
...
const WORKER_RELAY_SECRET = String(process.env.MC_RELAY_WORKER_SECRET || "");
const WORKER_RELAY_AUTH = String(process.env.MC_RELAY_WORKER_AUTH || "secret");
const WORKER_RELAY_ORIGIN = String(process.env.MCWEB_PUBLIC_ORIGIN || "https://wasm.click");
if (!["secret", "minecraft-session"].includes(WORKER_RELAY_AUTH)) {
  throw new Error("MC_RELAY_WORKER_AUTH must be secret or minecraft-session");
}
if (!["https://wasm.click", "https://minecraft.wasm.click"].includes(WORKER_RELAY_ORIGIN)) {
  throw new Error("MCWEB_PUBLIC_ORIGIN is not an accepted Worker relay origin");
}
/** Any syntactically valid public Minecraft target is available by default. */
...
const requestedTargets = String(readFlag(
  "allow",
  process.env.MC_RELAY_ALLOW ?? DEFAULT_MINECRAFT_TARGETS,
)).trim().toLowerCase();
const TARGET_POLICY = minecraftTargetPolicy(requestedTargets);
...
/**
 * Complete browser PKCE after Xbox has minted the XSTS proof. This route is
 * exposed only by the loopback dev server, which applies the same peer and
 * same-origin checks as launcher JSON upload before calling it.
 */
export async function handleMicrosoftBrowserAuth(req, res) {
  uploadedLauncherCredentials = null;
  validatedCredentialsCache = null;
  launcherUploadInvalidated = true;
  let validated;
  try {
    validated = await completeMicrosoftBrowserAuth(req);
  } catch (error) {
    const status = Number(error?.status);
    res.writeHead(status === 413 ? 413 : status === 401 || status === 403 ? 401 : 400, {
      "cache-control": "no-store",
      "content-type": "application/json",
    });
    res.end(JSON.stringify({
      authenticated: false,
      error: error?.message || "Microsoft sign-in could not verify Minecraft ownership",
    }));
    return;
  }
  installValidatedCredentials(validated);
  res.writeHead(200, { "cache-control": "no-store", "content-type": "application/json" });
  res.end(JSON.stringify({
    authenticated: true,
    mode: "online",
...
function installValidatedCredentials(validated) {
  uploadedLauncherCredentials = validated;
  launcherUploadInvalidated = false;
  validatedCredentialsCache = {
    credentials: validated,
...
export async function handleMinecraftSessionImport(req, res) {
  let body = Buffer.alloc(0);
  try {
    for await (const chunk of req) {
      body = Buffer.concat([body, chunk]);
      if (body.length > 16 * 1024) throw new Error("Minecraft session import is too large");
    }
    const value = JSON.parse(body.toString("utf8"));
    const credentials = {
      ok: true,
      provider: "official-launcher",
      token: String(value?.accessToken || ""),
      id: String(value?.profile?.id || "").replaceAll("-", "").toLowerCase(),
      name: String(value?.profile?.name || ""),
      expiresAt: Number(value?.expiresAt),
    };
    const validated = await validateLauncherCredentials(credentials);
    if (!validated.ok) throw new Error(validated.message || "Minecraft session validation failed");
    installValidatedCredentials({ ...validated, provider: "microsoft-browser" });
    res.writeHead(200, { "cache-control": "no-store", "content-type": "application/json" });
    res.end(JSON.stringify({
      authenticated: true,
      mode: "online",
    ...
    res.writeHead(401, { "cache-control": "no-store", "content-type": "application/json" });
    res.end(JSON.stringify({ authenticated: false, error: error?.message || "Minecraft session validation failed" }));
  }
}
...
export function handleMinecraftGatewayUpgrade(req, socket) {
  ...
  async function handleEncryptionRequest(body) {
    ...
    if (typeof upstream?.authorizeMinecraftSession === "function") {
      await upstream.authorizeMinecraftSession(hash);
    }
  ...
  try {
    ...
    // Worker mode never opens a direct TCP socket from this process. The
    // Cloudflare Worker owns the requested target's DNS/TCP connection.
    const addressReady = Promise.resolve(null);
    Promise.all([addressReady, authReady]).then(([, credentials]) => {
      if (closed) return;
      if (credentials?.error) throw new Error(credentials.error);
      const workerCredential = WORKER_RELAY_AUTH === "minecraft-session" ? "" : WORKER_RELAY_SECRET;
      const connected = new WorkerTcpUpstream(
        WORKER_RELAY_URL,
        resolved.host,
        resolved.port,
        workerCredential,
        {
          origin: WORKER_RELAY_ORIGIN,
          minecraftSession: WORKER_RELAY_AUTH === "minecraft-session" ? {
            profileId: credentials.id,
            profileName: credentials.name,
          } : null,
        },
      );
      attachUpstream(connected);
      connected.connect();
...
export function minecraftGatewayInfo() {
  return {
    authMode: AUTH_MODE,
    interactiveAuth: Boolean(process.env.MCWEB_MS_CLIENT_ID)
      || process.env.MCWEB_BROWSER_AUTH_PROXY === "1",
    workerRelay: true,
    workerRelayReady: WORKER_RELAY_AUTH === "minecraft-session"
      || WORKER_RELAY_SECRET.length >= WORKER_RELAY_MIN_SECRET_LENGTH,
    workerRelayOrigin: "https://tcp.wasm.click",
