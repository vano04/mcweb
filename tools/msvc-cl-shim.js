...
function reportSpawnFailure(result) {
  ...
  const message = `cl-shim: could not start ${cc}${code}: ${result.error.message}`;
...
if (preprocess) {
  const result = spawnSync(cc, ccArgs, { encoding: "utf8", windowsHide: true });
  reportSpawnFailure(result);
...
// GraalVM starts cl.exe with redirected output handles. Pipe clang output
// through this process instead of passing those handles to a second process,
// and preserve Node's spawn error if clang cannot start.
const result = spawnSync(cc, ccArgs, { encoding: "utf8", windowsHide: true });
reportSpawnFailure(result);
