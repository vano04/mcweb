...
import {
  ...
  handleMicrosoftBrowserAuth,
  handleMinecraftSessionImport,
...
const microsoftClientId = String(process.env.MCWEB_MS_CLIENT_ID || "").trim();
if (microsoftClientId && !/^[0-9a-f-]{36}$/i.test(microsoftClientId)) {
  throw new Error("MCWEB_MS_CLIENT_ID must be an Entra application client ID");
}

function loopbackOrigin(request) {
  try {
    const parsed = new URL(`http://${String(request.headers.host || "")}`);
    if (parsed.hostname !== "localhost" && !isLoopbackAddress(parsed.hostname)) return null;
...
const server = createServer((request, response) => {
  ...
  if (pathname === "/mcweb/config.json" && request.method === "GET") {
    ...
    response.end(JSON.stringify({
      ...
      auth: {
        ...
        interactive: gatewayInfo.interactiveAuth && Boolean(origin),
        sessionPath: "/mcweb/auth/session",
        launcherUploadPath: "/mcweb/auth/launcher-accounts",
        microsoftClientId: gatewayInfo.interactiveAuth && origin ? microsoftClientId || null : null,
        microsoftRedirectUri: gatewayInfo.interactiveAuth && origin ? `${origin}/auth/callback.html` : null,
        microsoftCompletePath: gatewayInfo.interactiveAuth && origin ? "/mcweb/auth/microsoft/complete" : null,
      ...
      gateway: {
        ...
        transport: gatewayInfo.workerRelay ? "cloudflare-worker" : "direct-tcp",
        relayOrigin: gatewayInfo.workerRelayOrigin,
        relayConfigured: gatewayInfo.workerRelayReady,
      },
    }));
    return;
  }
  ...
  if (pathname === "/mcweb/auth/microsoft/complete" && request.method === "POST") {
    if (!gatewayInfo.interactiveAuth
        || !trustedLauncherUploadPeer(request) || !sameOriginRequest(request)) {
      response.writeHead(403, { "Content-Type": "application/json; charset=utf-8" });
      response.end(JSON.stringify({ authenticated: false, error: "local same-origin Microsoft sign-in only" }));
      return;
    }
    handleMicrosoftBrowserAuth(request, response).catch(() => {
      if (!response.headersSent) {
        response.writeHead(503, { "cache-control": "no-store", "content-type": "application/json" });
      }
      response.end(JSON.stringify({ authenticated: false, error: "Microsoft sign-in could not be completed" }));
    });
    return;
  }

  if (pathname === "/mcweb/auth/minecraft-session" && request.method === "POST") {
    if (process.env.MCWEB_BROWSER_AUTH_PROXY !== "1"
        || !trustedLauncherUploadPeer(request) || !sameOriginRequest(request)) {
      response.writeHead(403, { "Content-Type": "application/json; charset=utf-8" });
      response.end(JSON.stringify({ authenticated: false, error: "trusted production companion only" }));
      return;
    }
    handleMinecraftSessionImport(request, response).catch(() => {
      if (!response.headersSent) response.writeHead(503, { "content-type": "application/json" });
      response.end(JSON.stringify({ authenticated: false, error: "Minecraft session import failed" }));
    });
    return;
  }
  ...
  // the local runtime and audio host can address them without adding generated
...
server.listen(port, host, () => {
  ...
  console.log(`Server-side Minecraft TCP bridge: ws://${shownHost}:${listeningPort}/mcweb/socket`);
    ...
    console.log(`Minecraft TCP upstream: ${gatewayInfo.workerRelayOrigin}`);
    if (!gatewayInfo.workerRelayReady) {
      console.warn("WARNING: MC_RELAY_WORKER_SECRET is not set; sign-in works but multiplayer is locked");
    }
