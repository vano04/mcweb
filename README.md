...
Set the approved Entra application client ID before starting the local server:

```sh
MC_WEB_PORT=4199 \
MCWEB_MS_CLIENT_ID=1d50bf3c-09c8-4378-9a47-7a89e34eb140 \
node tools/dev-server.mjs
```

Register `http://localhost:4199/auth/callback.html` as a **Single-page
application** redirect URI in Entra. The page uses authorization code plus
PKCE for Microsoft and Xbox. Microsoft/Xbox tokens stay in browser memory;
only the short-lived XSTS proof is sent to the same-origin loopback helper for
Minecraft's non-CORS ownership/profile checks and authenticated gateway.
Open the test launcher at `http://localhost:4199/` so the browser origin exactly
matches that Entra redirect.

Launcher JSON remains available beside Microsoft sign-in. The local Node
process looks for an active Microsoft account in the official Launcher file.
On macOS, it tries PrismLauncher if the official file is absent or unusable.
...
This dev lane accepts any valid public Minecraft server selected in the game.
The local auth-aware Node gateway terminates Minecraft login, encryption,
compression, and framing, then carries only opaque TCP bytes through
`wss://tcp.wasm.click/mcweb/socket`. It never opens a direct TCP fallback.
Set `MC_RELAY_ALLOW` to an exact comma-separated `host:port` list only when you
want to restrict the available servers; the default is `*` for public targets.

Supply the same local-only relay secret that is stored in the Cloudflare
Worker's `RELAY_SECRET` binding:

```sh
MC_WEB_PORT=4199 \
MCWEB_MS_CLIENT_ID=1d50bf3c-09c8-4378-9a47-7a89e34eb140 \
MC_RELAY_WORKER_SECRET=<local-only-secret> \
node tools/dev-server.mjs
```
...
```powershell
$env:MC_WEB_PORT = '4199'
$env:MCWEB_MS_CLIENT_ID = '1d50bf3c-09c8-4378-9a47-7a89e34eb140'
$env:MC_RELAY_WORKER_SECRET = '<local-only-secret>'
node tools\dev-server.mjs
```

The secret is sent only in the Node-to-Worker WebSocket authorization header.
It must never be put in browser JavaScript, a URL, or this repository. The
browser still connects only to the same-origin `/mcweb/socket` endpoint.
